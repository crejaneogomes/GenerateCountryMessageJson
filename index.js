...
for(i = 0; i <pais.length; i++){
  var col1 = pais[i].nome_pais_int;
  var columnName = 'id';
  var col2 = 'defaultMessage';
  json[columnName] = 'DataSections.ResidenceData.'+pais[i].nome_pais_int;
  json[col2] = pais[i].nome_pais;
  jsonPai[col1] = json;
...
const fs = require('fs')
fs.writeFile ("input.json", JSON.stringify(vetorfinal), function(err) {
  if (err) throw err;
...
var vetorfinal = [];
for(i = 0; i <pais.length; i++){
  var primeira = pais[i].nome_pais_int.replace(/^\w/, (c) => c.toLowerCase());
  var pais_ingles = (primeira.replace(/\s/g,'').replace('&','').replace('(','').replace(')','').replace('Å','').replace('é','').replace('.',''));
  var pais_ingles_diferente = (primeira.replace(/\s/g,'_').replace('&','').replace('(','').replace(')','').replace('Å','').replace('é','').replace('.',''));
  ...
  console.log(pais_ingles_diferente.toUpperCase()+': \''+pais_ingles_diferente.toUpperCase()+'\',');
}
